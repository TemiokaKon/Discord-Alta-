...
function connectToSocketIO() {
    if (typeof io !== 'undefined') {
        ...
        socket.on('connect', () => {
            ...
            if (window.toast) {
                window.toast.success('Подключено', 'Успешно подключено к серверу');
            }
        });
        ...
        socket.on('connect_error', (error) => {
            ...
            if (window.toast) {
                window.toast.error('Ошибка подключения', error.message || 'Не удалось подключиться к серверу');
            }
        });
...
async function loadFriends() {
    try {
        ...
        if (!response.ok || !payload?.success) {
            console.error('Error loading friends:', response.status, payload);
            displayFriends([]);
            populateDMList([]);
            return;
        }
...
window.sendFriendRequest = async function(friendId) {
    try {
        ...
        alert('Не удалось отправить запрос дружбы');
    }
};
...
async function initiateCall(friendId, type) {
    try {
        ...
        // Update call header
        document.querySelector('.call-channel-name').textContent = `Звоним...`;
...
async function loadUserServers() {
    try {
        ...
        if (!response.ok || !payload?.success) {
            console.error('Error loading servers:', response.status, payload);
            servers = [];
            return;
        }
...
async function createNewServer() {
    // Use the new server create modal instead of prompt
    if (window.serverCreateModal) {
        window.serverCreateModal.show();
        ...
        window.onServerCreated = async (serverData) => {
            try {
                ...
                if (response.ok) {
                    ...
                    // Use accessibility manager to announce
                    if (window.accessibilityManager) {
                        window.accessibilityManager.announceSuccess(`Сервер ${serverData.name} создан`);
                    }
                }
            } catch (error) {
...
function switchChannel(channelName) {
    ...
    // Fix: Add null check for currentChannelName element
    ...
    // Fix: Add null check for messageInput element
...
async function joinVoiceChannel(channelName) {
    ...
    try {
        ...
        alert('Ошибка доступа к камере/микрофону. Пожалуйста, предоставьте разрешения.');
    ...
    if (screenStream) {
        ...
        try {
            ...
            if (error.name === 'NotAllowedError') {
                alert('Отказано в доступе к демонстрации экрана');
            } else {
                alert('Ошибка демонстрации экрана. Попробуйте ещё раз.');
            }
        }
    }
}
...
function initializeProfileModal() {
    ...
    if (userProfileBtn) {
        userProfileBtn.addEventListener('click', () => {
            if (window.profileModal && currentUser) {
                window.profileModal.show(currentUser.id);
            }
        });
...
function initializeKeyboardNavigation() {
    // Add keyboard support to server icons
    ...
    serverIcons.forEach((icon, index) => {
        icon.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'ArrowDown') {
                e.preventDefault();
                const nextIcon = serverIcons[index + 1];
                if (nextIcon) nextIcon.focus();
            } else if (e.key === 'ArrowUp') {
                e.preventDefault();
                const prevIcon = serverIcons[index - 1];
                if (prevIcon) prevIcon.focus();
            }
        });
    });
    
    // Add keyboard support to channels
    ...
    channels.forEach((channel, index) => {
        channel.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'ArrowDown') {
                ...
                const currentIndex = allChannels.indexOf(channel);
                const nextChannel = allChannels[currentIndex + 1];
                if (nextChannel) nextChannel.focus();
            } else if (e.key === 'ArrowUp') {
                e.preventDefault();
                const allChannels = Array.from(document.querySelectorAll('.channel'));
                const currentIndex = allChannels.indexOf(channel);
                const prevChannel = allChannels[currentIndex - 1];
                if (prevChannel) prevChannel.focus();
            }
        });
    });
    
    // Add keyboard support to friends tabs
